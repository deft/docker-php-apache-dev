...
RUN useradd deft -m -p deft -s /bin/bash && apt-get update && apt-get install -y --no-install-recommends \
    ...
    && docker-php-ext-install bcmath bz2 exif gd intl mbstring pdo_mysql soap zip \
    ...
    && echo "extension=/usr/local/lib/php/extensions/no-debug-non-zts-20100525/imagick.so" > /usr/local/etc/php/conf.d/imagick.ini \
    && rm -rf /tmp/pear \
